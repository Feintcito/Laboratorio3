    ...
    if (err) {
        ...
        db.run(`CREATE TABLE IF NOT EXISTS messages (
            ...
            if (err) {
                ...
                // --- AQUI SE AGREGAN LOS ALTER TABLE PARA LAS COLUMNAS DE ARCHIVOS ---
                db.run(`ALTER TABLE messages ADD COLUMN file_name TEXT`, (err) => {
                    if (err && !err.message.includes('duplicate column name')) {
                        console.error("Error al agregar columna 'file_name':", err.message);
                    } else if (!err) {
                        console.log("Columna 'file_name' verificada/agregada.");
                    }
                });
                db.run(`ALTER TABLE messages ADD COLUMN file_size INTEGER`, (err) => {
                    if (err && !err.message.includes('duplicate column name')) {
                        console.error("Error al agregar columna 'file_size':", err.message);
                    } else if (!err) {
                        console.log("Columna 'file_size' verificada/agregada.");
                    }
                });
                db.run(`ALTER TABLE messages ADD COLUMN file_type TEXT`, (err) => {
                    if (err && !err.message.includes('duplicate column name')) {
                        console.error("Error al agregar columna 'file_type':", err.message);
                    } else if (!err) {
                        console.log("Columna 'file_type' verificada/agregada.");
                    }
                });
                db.run(`ALTER TABLE messages ADD COLUMN file_data TEXT`, (err) => {
                    if (err && !err.message.includes('duplicate column name')) {
                        console.error("Error al agregar columna 'file_data':", err.message);
                    } else if (!err) {
                        console.log("Columna 'file_data' verificada/agregada.");
                    }
                });
                // --- FIN DE LA SECCIÓN DE ALTER TABLE ---
            }
        });
    }
});
...
app.post('/upload-file', upload.single('file'), (req, res) => {
    ...
    jwt.verify(token, JWT_SECRET, (err, decoded) => {
        ...
        const fileData = {
            ...
            data: req.file.buffer.toString('base64') // Archivo en base64
        };
...
io.on('connection', (socket) => {
    ...
    // Evento: Mensaje privado (MODIFICADO PARA MANEJAR ARCHIVOS)
    socket.on('private_message', async (data) => {
        // Desestructurar todos los campos, incluyendo los de archivo
        const { receiver, encryptedMessage, iv, messageType, fileData } = data; 
        const sender = socket.user.username;
        const chatId = generateChatId(sender, receiver);

        console.log(`Recibido private_message de ${sender} para ${receiver}. IV: ${iv}. Tipo de mensaje: ${messageType || 'text'}`);
        ...
        `, [sender, receiver, receiver, sender], (err, friendship) => {
            ...
            if (connectedUsers[receiver]) {
                io.to(connectedUsers[receiver]).emit('private_message', {
                    ...
                    iv: iv,
                    chatId: chatId,
                    messageType: messageType || 'text', // Asegurar que siempre haya un tipo
                    fileData: fileData // Se envía el objeto completo de fileData
                });
            ...
            const params = [
                chatId, sender, receiver, encryptedMessage, iv,
                messageType || 'text', // Usar el messageType recibido o 'text' por defecto
                fileData ? fileData.name : null,
                fileData ? fileData.size : null,
                fileData ? fileData.type : null,
                fileData ? fileData.data : null // El base64 del archivo
            ];
            ...
            db.run(query, params, function(err) {
                if (err) {
                    ...
                    console.log(`Mensaje/archivo guardado en DB (ID: ${this.lastID}).`);
                }
            });
        });
    });
